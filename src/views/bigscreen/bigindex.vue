<template>
  <fullScreen>
      <div id="index" :style="{ width: width + 'px', height: height + 'px' }">
        ...
        <div style="height: 800px;overflow-y: auto;">
          <el-row>
            <h3>年</h3>
            <panels v-if="selectionsY.length>0" :options.sync="selectionsY" :defaultvalue="valueY" ptype="yyyy" @changeVal="changeVal"></panels>
          </el-row>
          <el-row>
            <div>年选中值：{{valueY}}</div>
          </el-row>
          <el-row>
            <h3>月</h3>
            <panels v-if="selectionsM.length>0" :options.sync="selectionsM" :isMultiply="false"  :defaultvalue="valueM" ptype="mm" @changeVal="changeVal"></panels>
          </el-row>
          <el-row>
            <div>月选中值：{{valueM}}</div>
          </el-row>
          <el-row>
            <h3>日</h3>
            <panels v-if="selectionsD.length>0" :options.sync="selectionsD" :isMultiply="false"  :defaultvalue="valueD" ptype="dd" @changeVal="changeVal"></panels>
          </el-row>
          <el-row>
            <div>日选中值：{{valueD}}</div>
          </el-row>
      ...
      <!-- 放大缩小 -->
      <!--
      <line3DChart id="line3DChart" :height="divheight" :key="refreshkey"/>
      <button type="submit" @click="divheight = isbig?300:800;refreshkey = new Date().getTime();isbig = !isbig">{{isbig?'变小':'变大'}}</button>
      -->
      <!--
      <div class="type-legend" :style="'position: absolute;top:'+(isFullscreen ? '72%':'60%' )+';background: #01005b;border-radius: 5px;'">
            <span :class="legshow?'el-icon-caret-left':'el-icon-caret-right'" :style="'position: absolute;top:50%;color: #fff;cursor: pointer;z-index: 9;'+'right:'+(legshow?'0px':'-10px')+';'" @click="legshow = !legshow;"></span>
              ...
              <ul>
                  <li><a href="#">视频学习</a>
                      <ul>
                          <li><a href="#">css</a></li>
                          <li><a href="#">js</a></li>
                          <li><a href="#">jquery+jquery+jquery+jquery</a></li>
                      </ul>
                  </li>
                  <li><a href="#">案例学习</a></li>
                  <li><a href="#">交流平台</a></li>
...
import panels from '@/views/components/bigscreen/checkPanel';
...
import {getRoutes} from '@/api/routeview/routeview';
export default {
  name:"Avue Crud",
  data() {
    return {
      ...
      legendName:"ABCDEF",
      myLegendItems: [
              { title: '水系', color: '#00FFFF' },
              { title: '陆地', color: '#00FF00' },
              { title: '火山', color: '#FF0000' },
            ],
      tab_radio:"0",
      isFullscreen:false,
      legshow:true
  ...
  created() {
    ...
    //this.getDays(this.valueY[0].value,this.valueM[0].value);

    let ranges = getLegendRanges('concentration','forecast');
